...
Every message in fflint is looked up by a stable key at the moment a diagnostic is produced. The `t(key, params)` function resolves the key against the active catalog and returns `{ message, hint }`. If the key is not found in the catalog, it falls back to the raw params object (so existing rules keep working even with a partial catalog).

The catalog itself is a plain JavaScript object: keys are message IDs, values are either static strings or `(params) => string` functions for dynamic content.
