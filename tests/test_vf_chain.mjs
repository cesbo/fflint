...
// ═══════════════════════════════════════════════════════════════════════════════
console.log('\n\x1b[1m═══ Section 4b: L1 scale syntax errors ═══\x1b[0m')
// ═══════════════════════════════════════════════════════════════════════════════

{
  // Wrong 'x' separator inside the filter
  const errs = validate(parse(
    `ffmpeg -y -i \${i} -s 1920x1080 -vf "scale=1920x1080" -f mpegts \${o}`
  ))
  const r = errs.find(e => e.id === 'l1_vf_scale_syntax')
  assert('l1_vf_scale_syntax fires for x-separator', !!r && r.severity === 'error')
  assert('mentions wrong separator', r && /separator/i.test(r.message))
}

{
  // Same typo on hardware variant
  const errs = validate(parse(
    `ffmpeg -y -hwaccel cuda -i \${i} -vf "scale_cuda=1280x720" -c:v h264_nvenc -b:v 4M -f mpegts \${o}`
  ))
  const r = errs.find(e => e.id === 'l1_vf_scale_syntax')
  assert('l1 fires for scale_cuda typo', !!r && r.severity === 'error')
...

{
  ...
  const errs = validate(parse(
    `ffmpeg -y -i \${i} -vf "scale=1920" -f mpegts \${o}`
  ))
  const r = errs.find(e => e.id === 'l1_vf_scale_syntax')
  assert('l1 fires for missing height', !!r && r.severity === 'error')
  assert('mentions both dimensions', r && /width and height/i.test(r.message))
}

{
  // Codec absent + -s + -vf scale: optional-codec regression check
  // (frameSize must still flow through to rules)
  const errs = validate(parse(
    `ffmpeg -y -i \${i} -s 1920x1080 -vf "scale=1280:720" -f mpegts \${o}`
  ))
  const r = errs.find(e => e.group === 's_and_vf_scale_conflict')
